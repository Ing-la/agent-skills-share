...
4. **LLM 设计 blocks.json 与 xhs-copy**（一次产出）：
   ...
   - **blocks.json 刚性约束**：
     - **Cover**：text 不得超过 2 行（约 20 字），只放金句或主卖点，禁止长段
     - **每块 text**：建议 150–200 字，**每页可含多条要点**；多条要点之间用**双换行**（`\n\n`）分隔，每条会渲染成独立圆点行；用短句、换行、关键词前置，让版面充实
     ...
     - 分几张图、每块 title/text/emoji 由你专业判断。避免同一张图内 title 与 emoji 重复。**纯 hashtag 不生成图片**
